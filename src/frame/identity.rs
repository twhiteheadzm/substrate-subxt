...
pub mod events {
    ...
    pub const TRANSFERED_SUM: &str = "TransferredSum";
}
...
pub enum ClaimDataType {
    ...
    fn default() -> Self { ClaimDataType::String }
...
pub struct AddKycproviderArgs<T: Identity>{
    ...
    kycprovider: <T as System>::Address,
}
...
pub struct CreateDidArgs<T: Identity> {
    registry_id: <T as System>::Hash,
    referent: <T as System>::Address,
}

/// call the extrinsic.
pub fn create_did<T: Identity>( registry_id: <T as System>::Hash,
    referent: <T as System>::Address) -> Call<CreateDidArgs<T>> {
    Call::new(MODULE, calls::CREATE_DID, CreateDidArgs {
        registry_id, referent
    })
}
/// Arguments for creating a registry
#[derive(Encode)]
pub struct CreateClaimArgs<T: Identity> {
    ...
    issued:i64,
    expiry:i64,
    assertion:Vec<u8>, data_type:ClaimDataType,value: ClaimDataValue
}

/// call the extrinsic.
pub fn create_claim<T: Identity>( did_id: <T as System>::Hash,
    issued:i64,
    expiry:i64,
    assertion:Vec<u8>, data_type:ClaimDataType,value: ClaimDataValue) -> Call<CreateClaimArgs<T>> {
    Call::new(MODULE, calls::CREATE_CLAIM, CreateClaimArgs {
        did_id,
        issued,
        expiry,
        assertion,data_type,value
    })
}
/// Arguments for creating a registry
#[derive(Encode)]
pub struct VerifyClaimArgs<T: Identity>  {
    did_id: <T as System>::Hash, claim_id: <T as System>::Hash
}

/// call the extrinsic.
pub fn verify_claim<T: Identity>( did_id: <T as System>::Hash, claim_id: <T as System>::Hash) -> Call<VerifyClaimArgs<T>> {
    Call::new(MODULE, calls::VERIFY_CLAIM, VerifyClaimArgs { did_id, claim_id})
}
